...
- 擬似クリップ機能
  - 「擬似クリップ」とはYouTube公式のクリップ機能とは異なり、スクリプトを用いて擬似的にクリップ機能を実現する機能です。
  - YouTube公式の「クリップ機能」と名称や機能を差別化させるため、「擬似クリップ」と呼んでいます。
  - 擬似クリップでは、YouTubeの動画の特定の区間を何度も視聴したい場合に使用できます。
...
## LocalTubeのインストール
設定等は[LocalTubeのReadMe](https://github.com/portfoliokns/LocalTube)をご確認ください。
...
- "保存(localhost連携)"をクリックすると、擬似クリップ情報がLocalTubeに保存できます。
...
- 動画視聴の途中で、YouTube公式の広告動画が開始された場合、"擬似クリップ機能","フィルタリング機能","上下反転機能","コントロール表示/非表示機能"が初期化されます。
